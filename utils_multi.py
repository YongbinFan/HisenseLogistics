...
def separate_data():
    ...
    return df_h_8186, df_h_200, df_h_blank, df_q_8186, df_q_005, df_q_mess


def match_list_8186(df_h_8186, df_q_8186):
...
def work_h_8186(df_h_8186, match_list_1):
    df_list = []
    df_h_8186_match = pd.DataFrame(columns=df_h_8186.columns)
    df_list.append(df_h_8186_match)
...
def work_q_8186(df_q_8186, match_list_1):
    ...
    for info_dict in match_list_1:
        ...
        filter_df_q = df_q_8186[
            (df_q_8186["Grade"] == grade) &
            (df_q_8186["Product"] == product) &
            (df_q_8186["Cust. Reference"] == ref)
            ]
        df_list.append(filter_df_q)

    df_q_8186_match = pd.concat(df_list)

    mask = ~df_q_8186.index.isin(df_q_8186_match.index)
    df_q_8186_diff = df_q_8186[mask]
    return df_q_8186_match, df_q_8186_diff


def match_list_200(df_h_200, df_q_005):
...
def work_200_005_h(df_h_200, match_list_1):
    df_list = []
    df_h_200_match = pd.DataFrame(columns=df_h_200.columns)
    df_list.append(df_h_200_match)
    for info_dict in match_list_1:
        grade = info_dict["Grade"]
        des = info_dict["Material Description"]
